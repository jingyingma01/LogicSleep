...
class GraphUnet(nn.Module):
    def __init__(self, config):
        ...
        node_dim = config.num_node * config.num_patch
        for i in range(self.l_n):
            self.down_cnns.append(CNN(self.channels[i], self.channels[i+1], self.kernal_size[i],
                                      self.drop_p, feature_dim, node_dim, self.norm))
            self.down_cnns.append(CNN(self.channels[i+1], self.channels[i+1], self.kernal_size[i],
                                      self.drop_p, feature_dim, node_dim, self.norm))
            self.pools.append(nn.MaxPool2d(kernel_size = (1, int(1/self.cs[i])),
                                           stride = (1, int(1/self.cs[i])), return_indices=True))

            self.up_cnns.append(
                CNN(self.channels[i + 2], self.channels[i + 1], self.kernal_size[i],
                    self.drop_p, feature_dim, node_dim, self.norm))
            self.up_cnns.append(
                CNN(self.channels[i + 1], self.channels[i + 1], self.kernal_size[i],
                    self.drop_p, feature_dim, node_dim, self.norm))
            ...
            node_dim = int(node_dim * self.ks[i])
            self.up_cnns.append(CNN(self.channels[i+2], self.channels[i+1], self.kernal_size[i+1],
                                    self.drop_p, feature_dim, node_dim, self.norm))
            self.gunpools.append(gUnpool())
            self.unpools.append(Unpool(self.cs[i]))

        self.bottom_cnn1 = CNN(self.channels[-2], self.channels[-1], self.kernal_size[-1],
                               self.drop_p, feature_dim, node_dim, self.norm)
        self.bottom_cnn2 = CNN(self.channels[-1], self.channels[-1], self.kernal_size[-1],
                               self.drop_p, feature_dim, node_dim, self.norm)
        self.bottom_gcn = GCN(feature_dim, self.gcn_h[-1], feature_dim, self.act, self.drop_p)
        self.last_cnn = CNN(self.channels[1], 5, self.kernal_size[0],
                            self.drop_p, config.feat_dim, config.num_node * config.num_patch, self.norm)
...
class CNN(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, drop_p, feature_dim, norm_dim, norm):
        ...
        if norm == 'Batch':
            ...
            self.norm = nn.LayerNorm([out_channels, norm_dim, feature_dim])
        elif norm == 'Group':
            num_groups = 16
